...
switch (foodSelection) {
  case 1:
    ...
    switch (dishSelection) {
      ...
      default:
        alert(
          `That is unfortunately not on our menu, please refresh the page and try again! 🍕`
        )
        exit(1)
    }
...
switch (true) {
  case ageInput >= 16 && ageInput <= 120:
    age = "an adult"
    alert(
      `You are ordering ${age} size portion. That will be 150 SEK. Click 'OK' to continue 👉`
    )
    break
  case ageInput >= 0 && ageInput < 16:
    age = "a child"
    alert(
      `You are ordering ${age} size portion. That will be 75 SEK. Click 'OK' to continue 👉`
    )
    break
  default:
    alert(
      `That age is not possible, please refresh the page and try again with your real age! 🍕`
    )
    exit(1)
}
