...
alert(
  ...
  const userName = prompt(
    "Please enter your name", "Hungry Hugo"
  );
  ...
  switch(foodChoice) {
    case 1:
      foodChoice = "Pizza";
      alert(`${foodChoice}, what a great choice!`)
      break;
    case 2:
      foodChoice = "Pasta";
      alert(`${foodChoice}, what a great choice!`)
  ...
  console.log("first choice", foodChoice)
  
  // Step 3 - Subtype choice
  const pizzaSubchoice = [1 = "Vegetarian", 2= "Salami", 3= "Calzone"];
  const pastaSubchoice = ["Vegetarian", "Meat", "Seafood"];
  const saladSubchoice = ["Vegetarian", "Cheese", "Shrimp"];
  let foodSubchoice = [""];

  switch(foodChoice) {
    case "Pizza":
      foodSubchoice = pizzaSubchoice;
      prompt(`What kind of ${foodChoice} would you like to order? ${pizzaSubchoice} `)
      break;
    case "Pasta":
      foodSubchoice = pastaSubchoice;
      break;
    case "Salad":
      foodSubchoice = saladSubchoice;
      break;
    default: 
      alert("We're sorry, we don't have that, please start over!")
      exit(1);
