...
if (selectedCategory) {
    ...
    if (dishChoice >= 1 && dishChoice <= categoryDishes.length) {
        ...
        // Step 4 - Age need to nest step 4 and 5 because otherwise the variable selectedDish will not work, or it should have been defined at a higher level (I kind of dislike this with JS)
        ...
        if (age <= 18) {
            isChild = 'child sized';
            subtypeMessage = 'will be prepared for you';
        } else {
            isChild = 'adult sized';
            subtypeMessage = 'will be prepared for you';
        }
        ...
        let dishSize;
        if (age <= 18) {
            dishSize = parseInt(prompt(`One ${isChild} ${selectedDish.name} ${subtypeMessage}. That'll be €10. Are you sure you want to order this? \nEnter a number to confirm: \n1 - Yes \n2 - No`));
        } else {
            dishSize = parseInt(prompt(`One ${isChild} ${selectedDish.name} ${subtypeMessage}. That'll be €15. Are you sure you want to order this? \nEnter a number to confirm: \n1 - Yes \n2 - No`));
        }

        // Step 5 - Order confirmation

        if (dishSize === 1) {
            alert(`Thank you for your order! Your delicious ${isChild} ${selectedDish.name} is being prepared. See you soon!`);
        } else {
