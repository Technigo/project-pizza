...
if (isNaN(foodChoiceInput) || Number(foodChoiceInput) < 1 || Number(foodChoiceInput) > 4) {
...
if (isNaN(subMenuInput) || Number(subMenuInput) < 1 || Number(subMenuInput) > 3) {
...
const customerAge = prompt ("Should the portion be for a child or an adult? Detail your age:");

let confirmation;

if (customerAge <= 18) {
  confirmation = prompt(`One child portion ${foodChoice} with ${subMenuChoice} it is on preparation for you. That'll be 90SEK. Are you sure you want to order this?
  Select a number to confirm:
    1 - Of course!
    2 - Not really
  `)
} else {
  confirmation = prompt(`One adult portion ${foodChoice} with ${subMenuChoice} it is on preparation for you. That'll be 110SEK. Are you sure you want to order this?
  Select a number to confirm:
    1 - Of course!
    2 - Not really
  `)
